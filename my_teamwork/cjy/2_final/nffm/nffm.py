        ...
        for s in self.hparams.single_features:
            ...
            for i in range(self.batch_size*self.all_process):
                ...
                if i%self.all_process!=self.idx:
                    continue
                try:
                    temp.append(self.hparams.dict[s][int(ss)])
                except:
                    temp.append(0)
            single_ids[s]=temp
...
class Model(object):
    ...
    def build_graph(self, hparams):
        initializer=tf.contrib.layers.variance_scaling_initializer(factor=2.0, mode='FAN_AVG', uniform=False)
        #linear 
        emb_inp_v1={}
        for s in hparams.single_features:
        ...
        for s in hparams.mutil_features:
            emb_inp_v2[s]=tf.reduce_max(tf.gather(self.emb_v2[s], self.mulit_ids[s]),axis=1)  
        ...
        input_size=emb_rep.get_shape()[-1]
        hidden_outputs=emb_rep
        
        for idx in range(len(hparams.hidden_size)):
            W = tf.get_variable("W_"+str(idx), [input_size,hparams.hidden_size[idx]],initializer=initializer)
            hidden_outputs=tf.nn.relu(tf.tensordot(hidden_outputs,W,[[-1],[0]]))  
            hidden_outputs=self.batch_norm_layer(hidden_outputs,self.use_dropout,'norm'+str(idx))
        ...
        W_output = tf.get_variable("W_output", [input_size],initializer=initializer)
        b_output = tf.Variable(tf.constant(-3.5), dtype=np.float32)
        nn_logits=tf.tensordot(hidden_outputs,W_output,[[-1],[0]])+b_output  
        
        
        
        score=lr_logits+nn_logits
    ...
    def optimizer(self,hparams):
        ...
        if hparams.optimizer == "sgd":
            ...
            opt = tf.train.AdamOptimizer(self.lrate,beta1=0.9, beta2=0.999,
                                                        epsilon=1e-8)
        params = tf.trainable_variables()
